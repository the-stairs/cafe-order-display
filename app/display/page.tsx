...
function DisplayContent() {
  ...
  useEffect(() => {
    ...
    const unsubscribeTTL = onValue(configRef, (snapshot) => {
      if (snapshot.exists()) {
        ...
        console.log("Display - TTL 설정값 로드:", ttlValue);
      } else {
        // 기본값 사용
        const defaultTTL = 300000; // 5분
        setCurrentTTL(defaultTTL);
        console.log("Display - 기본 TTL 사용:", defaultTTL);
      }
    });
  ...
  return (
    <div className="min-h-screen bg-white py-8">
      {/* 상단 헤더 */}
      <header className="relative flex flex-col lg:flex-row lg:justify-between lg:items-center mb-8 px-6 lg:px-10 py-4 lg:h-20">
        {/* 왼쪽 로고 */}
        <div className="flex-shrink-0">
          <svg
            ...
            className="h-12 lg:h-[72px] w-auto" // 모바일: 48px(1.5배), 데스크톱: 72px(1.5배)
          >
        ...
        {/* 가운데 메인 타이틀 */}
        <div className="absolute left-1/2 transform -translate-x-1/2 text-center lg:block hidden">
          <div className="text-center">
            <h1 className="text-5xl lg:text-7xl font-bold text-[#0B0B0C] mb-2">
              준비 완료! ☕
            </h1>
          </div>
        </div>
        ...
        <div className="text-center mt-4 lg:hidden">
          <div className="text-center">
            <h1 className="text-4xl font-bold text-[#0B0B0C] mb-2">
              준비 완료! ☕
            </h1>
            <p className="text-lg text-[#FF9900] font-medium">
              음료를 가져가주세요
            </p>
        ...
        {/* 우측 컨트롤 영역 */}
        <div className="flex flex-wrap items-center gap-2 lg:gap-4 mt-4 lg:mt-0 lg:ml-auto">
          {/* 사운드 토글 버튼 */}
          <button
            onClick={toggleSound}
          ...
          <div className="flex items-center gap-3 text-xs text-gray-500">
            ...
            <span>TTL: {Math.round(currentTTL / (60 * 1000))}분</span>
            {serverTimeOffset !== 0 && (
              <span>Offset: {serverTimeOffset}ms</span>
            )}
          </div>
        </div>
      </header>
      ...
      <main className="flex-1 px-10" role="main" aria-label="주문 디스플레이">
        ...
        <div className="mb-6 px-4">
          <div className="flex items-center gap-4">
            ...
            <p className="text-xl lg:text-2xl text-[#FF9900] font-bold">
              Pick Up
            </p>
